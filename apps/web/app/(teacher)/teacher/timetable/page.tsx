    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100">Timetable</h1>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">View your weekly class schedule</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-4">
        {days.map((d) => (
          <div
            key={d}
            className={`bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-3 sm:p-4 shadow-sm ${
              d === todayName ? 'ring-2 ring-indigo-400 dark:ring-indigo-600' : ''
            }`}
          >
            <div className="font-semibold text-gray-900 dark:text-gray-100 mb-2 sm:mb-3 flex items-center justify-between text-sm sm:text-base">
              <span className="truncate">{d.slice(0, 3)}<span className="hidden sm:inline">{d.slice(3)}</span></span>
              {d === todayName && (
                <span className="text-xs bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 px-1.5 sm:px-2 py-0.5 rounded flex-shrink-0">
                  Today
                </span>
              )}
            </div>
            {(dataByDay[d] || []).length === 0 ? (
              <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 italic">No sessions</div>
            ) : (
              <div className="space-y-1.5 sm:space-y-2">
                {(dataByDay[d] || []).map((s) => (
                  <div
                    key={s.id}
                    className="p-2 sm:p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all ease-in-out bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900"
                  >
                    <div className="text-xs sm:text-sm text-gray-800 dark:text-gray-200 font-medium truncate">{s.subject}</div>
